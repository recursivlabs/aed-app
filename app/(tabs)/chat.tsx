...
function AuthenticatedChatScreen() {
  ...
  // produced ghost agents named "AED Connect" right after signup before the
    ...
    try {
      ...
      if (!publicUrl) {
        publicUrl = await uploadMediaBlob({
          ...
          sdk: prepared.sdkIdentity,
          onError: () => showToast('Voice note upload failed. Keep AED Connect open and tap Send to retry.', 'error'),
        }) || undefined;
      ...
      if (note.deliveryAttemptedAt) {
        ...
        showToast(
          priorStatus === 'unknown'
            ? 'Could not verify the earlier send. No duplicate was sent; keep AED Connect open and check again later.'
      ...
      if (outcome === 'not-started') {
        ...
        showToast('Voice note not sent. Keep AED Connect open and tap Send to retry.', 'error');
        return false;
      }
      ...
      showToast('Voice note outcome is unconfirmed. No duplicate was sent; keep AED Connect open and retry later.', 'error');
      return false;
    } catch {
      showToast('Voice note not sent. Keep AED Connect open and tap Send to retry.', 'error');
      return false;
    ...
    try {
      ...
      if (
        ...
        if (recording && saveVoiceRecording(recording, origin)) {
          showToast('Voice note kept in its original conversation while AED Connect stays open.', 'error');
        }
      }
    } finally {
